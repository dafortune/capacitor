...
function buildCPUTrigger({ threshold, events }) {
  ...
  const MIN_TRIGGER_STATE_TIME = 10000;
  ...
  function hasTriggered() {
    ...
    isTriggered = cpu > threshold || isTriggered && (cpu > recoveryThreshold) || triggeredAt && Date.now() < triggeredAt + MIN_TRIGGER_STATE_TIME;
...
module.exports = (server, { requestRateThreshold, cpuThreshold, minCapacity, healthCheck }) => {
  ...
  server.use((req, res, next) => {
    ...
    // Don't apply load shedding for the health check
